...
exports.reservation = async (req, res, next) => {
  ...
  let query = `insert into reservation(movie_id,seat_id,user_id,start_time) values (${movie_id},
    "${seat_id}",${user_id},"${start_time}")`;

  try {
    [result] = await connection.query(query);
    res.status(200).json({ success: true, 좌석: seat_id });
  } catch (e) {
    if (e.errno == 1062) {
...
exports.get_movie = async (req, res, next) => {
  ...
  let start_time = req.body.start_time;
  let query = `select m.title,r.start_time ,r.seat_id from movie as m
